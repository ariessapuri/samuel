            ...
            <header class="header navbar navbar-expand-sm">
                <ul class="navbar-item theme-brand flex-row text-center">
                    <li class="nav-item theme-logo">
                        <router-link to="/">
                            <img :src="imageSrc" class="navbar-logo" alt="logo" />
                        </router-link>
                    </li>
                </ul>
                ...
                <div class="navbar-item flex-row ms-md-auto">
                    ...
                    <div class="dropdown nav-item user-profile-dropdown btn-group">
                        ...
                        <ul class="dropdown-menu dropdown-menu-right" aria-labelledby="ddluser">
                            ...
                            <li role="presentation">
                                <router-link to="/" class="dropdown-item" @click="signOut">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                        fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
...
let logo = 'uploads/logo.jpg';
if (logoData) {
    try {
        const cleanedLogData = logoData.replace(/\\/g, '');
        const parsedLogo = JSON.parse(cleanedLogData);
        console.log(parsedLogo);
        if (parsedLogo) {
            logo = parsedLogo;
        }
    } catch (e) {
        console.warn("failed parsing", e);
    }
}

const imageSrc = ref(`${config.baseAPIURL}storage/${logo}`);
